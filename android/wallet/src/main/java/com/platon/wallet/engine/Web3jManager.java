...
public class Web3jManager {
    ...
    public void init(String url) {
        ...
        System.out.println("Web3jManager 1111  init url = "+ url);
    ...
    private TransactionManager getTransactionManager(Credentials credentials) {
        return new RawTransactionManager(mWeb3j, credentials, 103);
    }
    ...
    /*public BigInteger getEstimateGas(String from, String to, String data) {
        try {
            return Web3jManager.getInstance().getWeb3j().platonEstimateGas(com.platon.protocol.core.methods.request.Transaction.createEthCallTransaction(from, to, data)).send().getAmountUsed();
        } catch (Exception e) {
            //LogUtils.e(e.getMessage(),e.fillInStackTrace());
