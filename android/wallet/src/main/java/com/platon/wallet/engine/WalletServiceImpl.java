...
import com.platon.wallet.utils.JZMnemonicUtil;
...
class WalletServiceImpl implements WalletService {
    ...
    private Wallet generateWallet(ECKeyPair ecKeyPair, String name, String password) {
        try {
            ...
            return new Wallet.Builder()
                    ...
                    .name(name)
                    //.address(walletFile.getAddress().getTestnet())
                    .address(walletFile.getAddress().getMainnet())
    ...
    public Wallet importKeystore(String store, String name, String password) {
        try {
            ...
            System.out.println("ecKeyPair = "+ecKeyPair);
...
//        String[] avatarArray = App.getContext().getResources().getStringArray(R.array.wallet_avatar);
    ...
    public Wallet changePassword(ECKeyPair ecKeyPair, String newPassword, Wallet oldWallet, String mnemonic){
        try {

            String filename = JZWalletUtil.getWalletFileName(Numeric.toHexStringNoPrefix(ecKeyPair.getPublicKey()));
            WalletFile walletFile = org.web3j.crypto.Wallet.create(newPassword, ecKeyPair, N_STANDARD, P_STANDARD);
            if (walletFile != null) {
               String key =  JZWalletUtil.writeWalletFileAsString(walletFile);
               String keystorePath = filename;
               oldWallet.setKey(key);
               oldWallet.setKeystorePath(keystorePath);
               oldWallet.setMnemonic(JZWalletUtil.encryptMnemonic(key, mnemonic, newPassword));
                return oldWallet;
            }
